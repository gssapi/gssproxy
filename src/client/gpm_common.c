...
static int gpm_epoll_wait(struct gpm_ctx *gpmctx, uint32_t event_flags)
{
    ...
    if (epoll_ret < 0) {
        ...
        /* If ret == 0, then we definitely timed out. Else, if ret == -1 and
         * errno == EAGAIN or errno == EWOULDBLOCK, we're in a weird edge case
         * where epoll thinks the timer can be read, but it is blocking more;
         * treat it like a TIMEOUT and retry, as nothing around us would
         * handle EAGAIN from timer and retry it. */
        ret = ETIMEDOUT;
        goto done;
    }
